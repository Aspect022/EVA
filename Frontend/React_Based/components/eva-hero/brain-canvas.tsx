...
export function BrainCanvas({ scrollProgress, activeMode, className }: BrainCanvasProps) {
    ...
    // Regenerate nodes when mode or canvas size changes
    const regenerateNodes = useCallback(
        (w: number, h: number) => {
            nodesRef.current = createNodes(activeMode.visual.nodeCount, w, h);
            sizeRef.current = { w, h };
        },
        [activeMode.visual.nodeCount]
    );
    ...
    useEffect(() => {
        if (prevModeRef.current !== activeMode.id) {
            ...
            regenerateNodes(sizeRef.current.w, sizeRef.current.h);
        }
    }, [activeMode.id, regenerateNodes]);
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            time += 0.016 * activeMode.visual.pulseSpeed;
            ...
            for (const node of nodes) {
                const drift = Math.sin(time * node.speed + node.phase) * (6 + node.layer * 4);
                const driftY = Math.cos(time * node.speed * 0.7 + node.phase) * (4 + node.layer * 3);
            ...
            // Draw connections
            const maxConnDist = Math.min(w, h) * activeMode.visual.connectionDensity * 0.35;
            ctx.lineWidth = 0.5;
